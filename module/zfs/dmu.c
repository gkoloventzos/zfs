    ...
    void *tag, dmu_buf_t **dbp)
{
	...
	db = dbuf_hold(dn, blkid, tag, NULL, NULL);
...
dmu_spill_hold_by_dnode(dnode_t *dn, uint32_t flags, void *tag, dmu_buf_t **dbp)
{
	...
	db = dbuf_hold(dn, DMU_SPILL_BLKID, tag, NULL, NULL);
    ...
    int *rot, const char *name)
{
	...
	blkid = dbuf_whichblock(dn, 0, offset);
	for (i = 0; i < nblks; i++) {
		dmu_buf_impl_t *db = dbuf_hold(dn, blkid + i, tag, rot, name);
		if (db == NULL) {
			rw_exit(&dn->dn_struct_rwlock);
		...
		/* initiate async i/o */
		if (read)
			(void) dbuf_read(db, zio, dbuf_flags, rot, name);
		dbp[i] = &db->db;
	}

	if ((flags & DMU_READ_NO_PREFETCH) == 0 && read &&
	    length <= zfetch_array_rd_sz) {
		dmu_zfetch(&dn->dn_zfetch, blkid, nblks, rot, name);
	}
	rw_exit(&dn->dn_struct_rwlock);
    ...
    uint64_t length, int read, void *tag, int *numbufsp, dmu_buf_t ***dbpp)
{
	...
	err = dmu_buf_hold_array_by_dnode(dn, offset, length, read, tag,
	    numbufsp, dbpp, DMU_READ_PREFETCH, NULL, NULL);
    ...
    uint64_t len, zio_priority_t pri)
{
	...
	if (len == 0) {  /* they're interested in the bonus buffer */
		...
		dbuf_prefetch(dn, level, blkid, pri, 0, NULL, NULL);
		rw_exit(&dn->dn_struct_rwlock);
		return;
	}
	...
	if (nblks != 0) {
		...
		for (i = 0; i < nblks; i++)
			dbuf_prefetch(dn, level, blkid + i, pri, 0, NULL, NULL);
	}
    ...
    void *buf, uint32_t flags)
{
	...
	while (size > 0) {
		...
		err = dmu_buf_hold_array_by_dnode(dn, offset, mylen,
		    TRUE, FTAG, &numbufs, &dbp, flags, NULL, NULL);
		if (err)
			break;
...
#ifdef _KERNEL
static int
dmu_read_uio_dnode(dnode_t *dn, uio_t *uio, uint64_t size, int *rot, const char *name)
{
	dmu_buf_t **dbp;
	int numbufs, i, err;
	xuio_t *xuio = NULL;
 ...
 */
int
dmu_read_uio_dbuf(dmu_buf_t *zdb, uio_t *uio, uint64_t size, int *rot, const char *name)
{
...
dmu_read_uio(objset_t *os, uint64_t object, uio_t *uio, uint64_t size)
{
	...
	err = dmu_read_uio_dnode(dn, uio, size, NULL, NULL);
    ...
    dmu_tx_t *tx)
{
	...
	rw_enter(&dn->dn_struct_rwlock, RW_READER);
	blkid = dbuf_whichblock(dn, 0, offset);
	VERIFY((db = dbuf_hold(dn, blkid, FTAG, NULL, NULL)) != NULL);
	rw_exit(&dn->dn_struct_rwlock);
	DB_DNODE_EXIT(dbuf);
