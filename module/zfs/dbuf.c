...
static int __dbuf_hold_impl(struct dbuf_hold_impl_data *dh, int *rot, const char *);
...
int
dbuf_read(dmu_buf_impl_t *db, zio_t *zio, uint32_t flags, int *rot, const char *name)
{
	int err = 0;
    ...
    if (_myprint) {
        if (havepzio) {
            if (zio->name == NULL)
                ...
                printk(KERN_EMERG "[PRINT]Passed %s in %s %lld offset %lld\n", __FUNCTION__, zio->name, zio->io_timestamp, zio->io_offset);
        }
        else
	...
	if (db->db_state == DB_CACHED) {
		mutex_exit(&db->db_mtx);
		if (prefetch)
			dmu_zfetch(&dn->dn_zfetch, db->db_blkid, 1, rot, name);
		if ((flags & DB_RF_HAVESTRUCT) == 0)
			rw_exit(&dn->dn_struct_rwlock);
		DB_DNODE_EXIT(db);
	...
	if (db->db_level+1 < dn->dn_nlevels) {
		...
		if (db->db_parent == NULL || db->db_parent == dn->dn_dbuf) {
			...
			parent = dbuf_hold_level(dn, db->db_level+1,
			    db->db_blkid >> epbs, FTAG, NULL, NULL);
			ASSERT(parent != NULL);
			parent_held = TRUE;
		}
		if (drop_struct_lock)
...
dmu_buf_will_dirty(dmu_buf_t *db_fake, dmu_tx_t *tx)
{
	...
	(void) dbuf_read(db, NULL, rf, NULL, NULL);
	...
	*bpp = NULL;
	...
	if (level >= nlevels ||
		...
		err = dbuf_read(*parentp, NULL,
		    (DB_RF_HAVESTRUCT | DB_RF_NOPREFETCH | DB_RF_CANFAIL), rot, name);
		if (err) {
...
dbuf_prefetch(dnode_t *dn, int64_t level, uint64_t blkid, zio_priority_t prio,
    arc_flags_t aflags, int *rot, const char * name)
{
	blkptr_t bp;
	...
	while (curlevel < nlevels - 1) {
		...
		if (dbuf_hold_impl(dn, parent_level, parent_blkid,
		    FALSE, TRUE, FTAG, &db, NULL, NULL) == 0) {
 ...
 */
static int
__dbuf_hold_impl(struct dbuf_hold_impl_data *dh, int *rot, const char *name)
{
...
dbuf_hold_impl(dnode_t *dn, uint8_t level, uint64_t blkid,
    ...
    void *tag, dmu_buf_impl_t **dbp, int *rot, const char *name)
{
	...
	error = __dbuf_hold_impl(dh, rot, name);
...
dmu_buf_impl_t *
dbuf_hold(dnode_t *dn, uint64_t blkid, void *tag, int *rot, const char *name)
{
    if (_myprint)
        printk(KERN_EMERG "[PRINT]Passed %s in name %s\n", __FUNCTION__, name);
	return (dbuf_hold_level(dn, 0, blkid, tag, rot, name));
}

dmu_buf_impl_t *
dbuf_hold_level(dnode_t *dn, int level, uint64_t blkid, void *tag, int *rot, const char *name)
{
	dmu_buf_impl_t *db;
    int err;
    if (_myprint)
        printk(KERN_EMERG "[PRINT]Passed %s in name %s\n", __FUNCTION__, name);
	err = dbuf_hold_impl(dn, level, blkid, FALSE, FALSE, tag, &db, rot, name);
	return (err ? NULL : db);
}
